...
class HourBar extends Component {
	...
	render(){
		...
		for (let i = 0; i < hourArray.length; i++){
			...
			divArray[i] = <div key={hourArray[i]} style={{color:'#BEBEBE', backgroundColor:backgroundColor, padding:30}}  onClick={this.handleHourClick.bind(this, hourArray[i])}>{hourArray[i]}</div>
		}

		return <Row>
					<div style={{display:'flex', padding: 15, flexWrap:'wrap', justifyContent:'space-between', width:'100%'}}>
						{divArray}
					</div>
				</Row>
	}
